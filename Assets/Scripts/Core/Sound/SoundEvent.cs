...
public abstract class SoundEvent
{
    ...
    public static void CleanUpChannels()
    {
        ...
        foreach (int channelID in channelRefs.Keys)
        {
            ...
            // 否则会浪费底层资源
            if (channelRefs[channelID] == null)
            {
                Log.Hsz("Warning: Couldn't find sound - " + channelID);
                continue;
            }
...
public class PlaySFXEvent : SFXEvent
{
    public override int FireEvent(int id)
    {
        ...
        if (clip != null)
        {
            ...
            AudioChannel channel = Main.SoundManager.PlaySoundAt(clip, Vector3.zero, volume * SoundEvent.volumeScale, pitch);
...
public class StartSFXLoopEvent : SFXEvent
{
    public override int FireEvent(int id)
    {
        if (clip != null && Main.SoundManager != null)
        {
            ...
            AudioChannel channel = Main.SoundManager.PlaySoundAt(clip, Vector3.zero, volume * SoundEvent.volumeScale, pitch, true);
            channel.isMusic = false;
            id = GetSoundID();
...
public class StartMusicEvent : MusicEvent
{
    public override int FireEvent(int id)
    {
        if (clip != null && Main.SoundManager != null)
        {
            AudioChannel channel = Main.SoundManager.PlayMusic(clip, startEndVolume.x * SoundEvent.volumeScale, startEndVolume.y * SoundEvent.volumeScale, fadeTime, loop);
...
public class FadeMusicEvent : MusicEvent
{
    ...
    public override int FireEvent(int id)
    {
        if (Main.SoundManager != null)
        {
            ...
            if (channel != null)
            {
                Main.SoundManager.FadeMusic(channel, targetVolume * SoundEvent.volumeScale, fadeTime, true);
            }
            else
            {
...
public class StopMusicEvent : MusicEvent
{
    ...
    public override int FireEvent(int id)
    {
        if (Main.SoundManager != null)
        {
            ...
            if (channel != null)
            {
                Main.SoundManager.FadeMusic(channel, targetVolume * SoundEvent.volumeScale, fadeTime);
            }
            else
            {
